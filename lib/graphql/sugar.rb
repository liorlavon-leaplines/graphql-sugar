...
module GraphQL
  module Sugar
    ...
    def self.get_resolver_graphql_type(field_name)
      "Types::#{field_name.to_s.classify}".constantize
    end
    ...
    def self.get_column_details(model_class, column_name)
      column_details = model_class.attribute_types[column_name]
    ...
    def self.get_graphql_type(model_class, column_name, enforce_non_null: true) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
      ...
      type = if model_class.defined_enums.key?(column_name)
        ...
        #values = model_class.send(column_name.pluralize.to_sym)
        
        # x = GraphQL::EnumType.define do
        #   name "#{column_name.camelize}Type"
        #   for k,v in values
        #     value k, value: k
        #   end
        # end
      ...
      type = type.to_list_type if column_details.is_a?(ActiveRecord::ConnectionAdapters::PostgreSQL::OID::Array)

      if enforce_non_null
        if column_details.respond_to?(:null)
...
require 'graphql/sugar/define/can'
